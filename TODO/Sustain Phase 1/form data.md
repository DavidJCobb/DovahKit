...
* There are several cases where uses may be discarded during load. For example, a malformed package may contain PKDD (`TESDialoguePackageData`), which refers to a `TESTopic*`, followed by PKW3 (`TESUseWeaponPackageData`), which refers to a `TESObjectWEAP*`. In that scenario, the dialogue data would be cleared out during load, and we would -- in DovahKit's current design -- end up not tracking the fact that it has a discarded-during-load use of some `TESTopic*`. This feels... improper. If the `TESTopic` in question is deleted, we should update the PACK even if it doesn't *effectively* load the topic -- even if the use is discarded during load.
