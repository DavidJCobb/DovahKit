...
class DKColorPickerButton : public QPushButton {
   Q_OBJECT;
   Q_PROPERTY(QColor color    READ color    WRITE setColor    DESIGNABLE true USER true);
   Q_PROPERTY(bool   hasAlpha READ hasAlpha WRITE setHasAlpha DESIGNABLE true);
   public:
      DKColorPickerButton(QWidget* parent = nullptr);

      inline QColor color() const noexcept { return this->_color; }
      inline bool hasAlpha() const noexcept { return this->_hasAlpha; }

      QString colorName() const noexcept;
