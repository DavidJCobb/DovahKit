...
namespace cobb::qt::ini {
   ...
   bool Setting::_typeCheckValue(const QVariant& v) const noexcept {
      return v.canConvert(this->qMetaTypeID);
   }

   void Setting::setCurrentValue(const QVariant& v) noexcept {
      if (v.isValid() && !this->_typeCheckValue(v)) {
         return;
      }
