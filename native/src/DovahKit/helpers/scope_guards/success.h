...
namespace cobb::scope_guards {
   ...
   class success {
      ...
      public:
         template<typename FunctorArg>
            requires (
               !std::is_same_v<std::remove_cvref_t<FunctorArg>, success> &&
               std::is_constructible_v<Functor, FunctorArg>
            )
         explicit constexpr success(FunctorArg&& functor)
         ...
         constexpr success(success&& other)
   ...
   template<typename Functor>
   success(Functor a) -> success<Functor>;
}