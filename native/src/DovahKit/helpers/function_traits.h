...
namespace cobb {
   ...
   template<typename R, typename ...Args> struct function_traits<std::function<R(Args...)>> {
      ...
      template<size_t i> using arg_type = std::tuple_element<i, std::tuple<Args...>>::type;
   };
   ...
   template<auto func, size_t n> using type_of_nth_argument = function_traits<decltype(func)>::arg_type<n>;
