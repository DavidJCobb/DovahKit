#include "mat4_by_vec3_simd.h"
#include <intrin.h>

namespace cobb::glm {
   extern ::glm::vec4 mat4_by_vec3_simd(const ::glm::mat4& m, const ::glm::vec3& v) {
      __m128 simd_v;
      {
         auto ext = ::glm::vec4(v, 1.0F);
         simd_v = _mm_load_ps(&ext.x);
      }
      ...
      b = _mm_load_ps(&m[3].x); // multiply by v.w, which is assumed to be 1 here
