...
namespace dovahscript::api_helpers::subobject_property_helpers {
   ...
   int apply_table_via_lua_method(lua_State* L) {
      ...
      {
         auto message = verify_table<subobject_type, IsAssign, PropertyList>(L, 2);
         if (!message.empty())
            cobb::lua::error(L, "bad argument: %s", message.data());
      }
      if constexpr (utils::property_list_has_late_checks<PropertyList>) {
         verify_table_late<subobject_type, IsAssign, PropertyList>(L, 2, *self.form, &subobject);
      }
      self.before_edit();
      subobject_property_helpers::apply_table<subobject_type, IsAssign, PropertyList>(L, 2, *self.form, subobject);
      self.after_edit();
      return 0;
   }
