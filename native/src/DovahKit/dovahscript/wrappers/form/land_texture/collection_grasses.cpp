...
namespace {
   constexpr const char* collection_metatable_key = "collection<dovah.classes.land_texture.grasses>";
}
...
namespace {
   ...
   int member_function_insert(lua_State* L) {
      ...
      if (!lua_isnoneornil(L, pos_value)) {
         target = pull_form_stub_argument(L, pos_value, dovah::form_type::grass);
         if (!target)
   ...
   int set_item(lua_State* L) {
      ...
      if (!lua_isnoneornil(L, index_value)) {
         target = pull_form_stub_argument(L, index_value, dovah::form_type::grass);
         if (!target)
            cobb::lua::error(L, "expected a form or nil");
      }
      auto& self = get_collection_wrapper(L);
...
namespace dovahscript::wrappers::collections {
   extern const collection_definition_params land_texture_grasses = {
