...
#include "dovah/forms/Actor.h"

namespace {
   using namespace dovahscript;
   using cls          = wrappers::actor;
   ...
   namespace _setters {
      int base_form(lua_State* L) {
         ...
         if (value->form_type != dovah::form_type::actor_base) {
            cobb::lua::argerror(L, 2, "the provided form is not an ActorBase; you cannot change an Actor's base form to a non-ActorBase");
         }
         if (!form)
            return 0;
         self.before_edit();
