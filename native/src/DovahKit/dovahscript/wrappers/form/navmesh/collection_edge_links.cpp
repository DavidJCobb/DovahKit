#include "./collection_edge_links.h"
...
namespace {
   constexpr const char* collection_metatable_key = "collection<dovah.classes.navmesh.edge_links>";
}
...
namespace {
   ...
   int lookup_item_by_index(lua_State* L) {
      ...
      assert(out.parts[0].signature == wrapper_part_types::navmesh_edge_link);
      out.into_collection(i);
      return core::subsystems::userdata::get().push(L, out, wrappers::navmesh_edge_link::metatable_key);
   }
}

namespace dovahscript::wrappers::collections {
   extern const collection_definition_params navmesh_edge_links = {
