#include "./collection_triangles.h"
...
namespace {
   constexpr const char* collection_metatable_key = "collection<dovah.classes.navmesh.triangles>";
}
...
namespace {
   ...
   int lookup_item_by_index(lua_State* L) {
      ...
      assert(out.parts[0].signature == wrapper_part_types::navmesh_triangle);
...
namespace dovahscript::wrappers::collections {
   extern const collection_definition_params navmesh_triangles = {
