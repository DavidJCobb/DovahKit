#include "./collection_door_links.h"
...
namespace {
   constexpr const char* collection_metatable_key = "collection<dovah.classes.navmesh.door_links>";
}
...
namespace {
   ...
   int lookup_item_by_index(lua_State* L) {
      ...
      assert(out.parts[0].signature == wrapper_part_types::navmesh_door_link);
      out.into_collection(i);
      return core::subsystems::userdata::get().push(L, out, wrappers::navmesh_door_link::metatable_key);
   }
}

namespace dovahscript::wrappers::collections {
   extern const collection_definition_params navmesh_door_links = {
