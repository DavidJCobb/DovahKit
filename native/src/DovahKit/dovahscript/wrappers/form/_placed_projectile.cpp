...
#include "dovah/forms/PlacedArrow.h"

namespace {
   using namespace dovahscript;
   using cls          = wrappers::_placed_projectile;
   ...
   namespace _setters {
      int base_form(lua_State* L) {
         ...
         if (value->form_type != dovah::form_type::projectile) {
            cobb::lua::argerror(L, 2, "the provided form is not an Projectile; you cannot change a placed projectile's base form to a non-Projectile");
         }
         if (!form)
            return 0;
         self.before_edit();
