...
namespace {
   constexpr const char* collection_metatable_key = "collection<dovah.classes.ui.table_view_col.cells>";
}
   ...
   int get_collection_length(lua_State* L) {
      ...
      int   result = 0;
      {
         ...
         task->handler = [observer, &result]() {
            result = get_model(*observer).rowCount();
         };
   ...
   int lookup_item_by_index(lua_State* L) {
      ...
      observer_t* result = nullptr;
      {
         ...
         task->handler = [index, observer, &result]() {
            ...
            auto* item  = model.item(index, observer->col);
            if (!item)
               return;
...
namespace dovahscript::wrappers::ui::collections {
   extern const collection_definition_params table_view_col_cell_list = {
