...
namespace {
   ...
   namespace _methods {
      ...
      int remove_layer(lua_State* L) {
         ...
         if (is_wrong_parent)
            cobb::lua::error(L, "the specified child layer isn't actually a child of the specified layer group");
   ...
   namespace _setters {
      ...
      int name(lua_State* L) {
         ...
         api_helpers::set_widget_property<QObject, QObject, const QString&, QString>(self.canvas_entity, &QObject::setObjectName, value);
         return 0;
      }
      int opacity(lua_State* L) {
         auto& self  = get_wrapper_for_thiscall<cls>(L);
