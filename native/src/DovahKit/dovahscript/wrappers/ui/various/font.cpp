...
namespace {
   QFont _get_font(const dovahscript::wrapper& w) {
      switch (w.type) {
         using wt = dovahscript::wrapper_type;
         case wt::widget: // widget
            if (auto* u = w.widget) {
               assert(w.parts[0].signature == dovahscript::wrapper_part_types::ui_font_data);
               return u->font();
            }
            break;
         case wt::canvas_layer_data:
            if (auto* d = w.canvas_layer_data) {
               if (auto* tld = qobject_cast<DovahscriptCanvasWidgetLayerDataText*>(d)) {
                  assert(w.parts[0].signature == dovahscript::wrapper_part_types::ui_font_data);
                  return tld->font;
               }
            }
   ...
   void _set_font(dovahscript::wrapper& w, QFont f) {
      switch (w.type) {
         using wt = dovahscript::wrapper_type;
         case wt::widget: // widget
            if (auto* u = w.widget) {
...
namespace dovahscript::impl::font_properties {
   namespace {
...
namespace {
   ...
   template<typename T> QVariant simple_pull(lua_State* L, int stack_pos) {
      if constexpr (std::is_same_v<T, int>) {
         ...
         if (!isnum) {
            cobb::lua::error(L, "integer or nil expected");
         }
         return QVariant::fromValue<int>(value);
      } else if constexpr (std::is_same_v<T, const char*>) {
         if (!lua_isstring(L, stack_pos)) {
            cobb::lua::error(L, "string or nil expected");
         }
         ...
         if (!lua_isnumber(L, stack_pos)) {
            cobb::lua::error(L, "number or nil expected");
         }
   ...
   QVariant pull_indexed_integer(lua_State* L, int stack_pos) {
      ...
      int value = lua_tointegerx(L, stack_pos, &isnum);
      if (!isnum)
         cobb::lua::error(L, "integer expected");
      --value;
      return QVariant::fromValue<int>(value);
   }
}
         ...
         QVariant pull(lua_State* L, int stack_pos) {
            ...
            for(int i = 0; i < size; ++i) {
               ...
               if (!_handle_quotes(family, quoted)) {
                  cobb::lua::error(L, "incorrect use of enclosing quotes");
               }
               if (!quoted) {
                  if (_is_generic(family) && i != size - 1) {
                     cobb::lua::error(L, "a font family list can only contain one generic family, and it must be at the end of the list; to use a font actually named \"%s\", enclose it in quotes", family);
                  }
               }
            }
            //
      ...
      namespace weight {
         QVariant pull(lua_State* L, int stack_pos) {
            ...
            if (value <= 0 || value > 100)
               cobb::lua::error(L, "weight must be between 1 and 100 inclusive");
            --value;
            return QVariant::fromValue<int>(value);
         }
      }
      namespace width {
         ...
         QVariant pull(lua_State* L, int stack_pos) {
            ...
            if (!isnum)
               cobb::lua::error(L, "integer or nil expected");
            if (value <= 0)
               cobb::lua::error(L, "font stretch must be greater than zero");
            if (value > 4000)
               cobb::lua::error(L, "font stretch cannot be greater than 4000");
            return QVariant::fromValue<int>(value);
         }
      }
   }
...
namespace dovahscript::wrappers::ui {
   ...
   using fph = dovahscript::impl::font_properties::handler;
   /*static*/ const dovahscript::impl::font_properties::handler_set cls::fph_handlers = {{
      fph{ 
   ...
   /*static*/ QFont cls::pull(lua_State* L, int stack_pos) {
      ...
      if (!lua_istable(L, stack_pos) && !lua_isuserdata(L, stack_pos))
         cobb::lua::error(L, "nil, font, or table expected");
