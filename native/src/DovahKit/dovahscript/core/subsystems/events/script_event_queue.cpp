#include "script_event_queue.h"
#include "script_event.h"
#include "../events.h"

namespace dovahscript::impl {
   void script_event_queue::clear() {
   ...
   size_t script_event_queue::process() {
      //
      ...
      auto& events_s = core::subsystems::events::get();
      for (auto* event : local) {
         events_s.fire_event(event->target, event->event_name.c_str(), event->listener_name.c_str(), event->params);
         delete event;
      }
      return local.size();
   }
   void script_event_queue::push_back(script_event* e) {
      auto guard = std::lock_guard(this->lock);
      this->list.push_back(e);
   }

   void script_event_queue::forget_about(QObject& target) {
      auto  guard = std::lock_guard(this->lock);
      auto& list  = this->list;
      list.erase(
         std::remove_if(
            list.begin(),
            list.end(),
            [&target](const script_event* entry) {
