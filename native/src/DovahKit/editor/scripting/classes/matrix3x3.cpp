...
namespace {
   ...
   using cls = classes::matrix3x3;

   namespace _singleton_functions {
      luastackchange_t construct_from_extrinsic_zyx(lua_State* L) {
      ...
      luastackchange_t construct_from_x(lua_State* L) {
      ...
      luastackchange_t construct_from_y(lua_State* L) {
         luaL_argcheck(L, lua_isnumber(L, 1), 1, "expected radians (number)");
         double radians   = lua_tonumber(L, 1);
         bool   righthand = lua_toboolean(L, 2);
      ...
      luastackchange_t construct_from_z(lua_State* L) {
         luaL_argcheck(L, lua_isnumber(L, 1), 1, "expected radians (number)");
         double radians   = lua_tonumber(L, 1);
         bool   righthand = lua_toboolean(L, 2);
      ...
      luastackchange_t identity(lua_State* L) {
   ...
   namespace _methods {
      luastackchange_t __mul(lua_State* L) {
         ...
         if (rawtype == LUA_TTABLE || rawtype == LUA_TUSERDATA) {
            ...
            if (classes::vector3::check_arg_type(L, 2)) {
               ...
               lua_settop(L, 0);
               v = a * v;
               classes::vector3::push_new_instance(L, v[0], v[1], v[2]);
               return 1;
            }
            if (classes::matrix3x3::check_arg_type(L, 2)) {
      ...
      luastackchange_t mul(lua_State* L) {
         lua_settop(L, 2);
         //
         cls::require_self_type(L);
         auto rawtype = lua_type(L, 2);
      ...
      luastackchange_t trace(lua_State* L) {
         cls::require_self_type(L);
         lua_settop(L, 1);
         //
         auto mat = cls::extract_from_stack(L, 1);
      ...
      luastackchange_t transpose(lua_State* L) {
         cls::require_self_type(L);
         lua_settop(L, 1);
         //
      ...
      luastackchange_t transpose_in_place(lua_State* L) {
         cls::require_self_type(L);
         lua_settop(L, 1);
         //
         auto m = cls::extract_from_stack(L, 1);
...
namespace editor_script::classes {
   /*static*/ std::initializer_list<luaL_Reg> matrix3x3::metatable_methods = {
   ...
   /*static*/ cobb::rotation_matrix matrix3x3::extract_from_stack(lua_State* L, int stack_pos) {
      cobb::rotation_matrix raw;
   ...
   /*static*/ void matrix3x3::push_new_instance(lua_State* L, const cobb::rotation_matrix& raw) {
   ...
   /*static*/ void matrix3x3::setup(lua_State* L) {
