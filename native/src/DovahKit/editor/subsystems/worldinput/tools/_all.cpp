...
namespace dovahkit::subsystems::worldinput {
   all_tool_instances::all_tool_instances() {
      size_t index = 0;
      all_tools::for_each<[]<typename Argument>(all_tool_instances::list_type& list, size_t& index) {
         static_assert(std::is_base_of_v<tools::base, Argument>);
         ...
         std::forward<list_type&>(this->pointers),
         std::forward<size_t&>(index)
      );
   }
