...
namespace dovahkit::subsystems::audio::utils {
   extern std::shared_ptr<sound_definition> load_asset_as_sound_definition(std::filesystem::path path) {
      ...
      {
         const auto* file_data = file_ptr->data();
         const auto  file_size = file_ptr->size();
         sound_definition_classes::for_each_until_true([&file_ptr, file_data, file_size, &result]<typename T>() {
