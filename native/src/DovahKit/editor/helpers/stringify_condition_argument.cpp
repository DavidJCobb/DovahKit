...
namespace {
   QString _stringify_event_function(uint16_t func) {
      switch (func) {
      ...
      return QObject::tr("<event function:%1>").arg(func);
   }
   QString _stringify_event_member(const context& context, uint16_t member) {
   ...
   QString _stringify_working_parameter(
      bool& incomplete_information,
      const context& context,
      const dovah::conditions::parameter_typeinfo* type,
      const dovah::conditions::parameter_underlying_type under,
      const dovah::loaded_forms::components::conditions::working_parameter& value
   ) {
      if (std::holds_alternative<uint32_t>(value)) {
         auto dword = std::get<uint32_t>(value);
         if (type == &dovah::conditions::parameter_types::ActorValue) {
            QString out = editor_helpers::actor_value_index_to_name(dword);
            if (!out.isEmpty())
               return out;
         }
         switch (under) {
      ...
      #if _DEBUG
         __debugbreak(); // unhandled condition parameter variant type
      #endif
      return {};
   }
}
...
namespace editor_helpers {
   extern QString stringify_condition_argument(
      ...
      if (func && func->uses_event_data) {
         ...
         switch (arg_index) {
            case 0:
               return _stringify_event_function(params.function);
            case 1:
               return _stringify_event_member(context, params.member);
            case 2:
      ...
      return _stringify_working_parameter(
   ...
   extern QString stringify_condition_argument(
      ...
      if (func && func->uses_event_data) {
         if (cnd.event_parameters.has_value()) {
            const auto& params = cnd.event_parameters.value();
            switch (arg_index) {
               case 0:
                  return _stringify_event_function(params.function);
               case 1:
      ...
      //
      auto* type  = cnd.get_effective_argument_typeinfo(arg_index);
      auto  under = cnd.get_argument_underlying_type(arg_index);
