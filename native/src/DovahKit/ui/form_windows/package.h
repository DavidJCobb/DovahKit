   ...
   public FormEditDialogMixin<dovah::loaded_forms::Package, true>
{
   ...
   protected:
      ...
      void _set_package_template(dovah::form_stub*);

      bool _uses_package_template();

      #pragma region Package Data UI
         std::optional<size_t> _selected_packdata_row() const;

         void _move_selected_packdata(int by);

         void _on_packdata_selection_changed();
         void _pull_packdata_to_ui(size_t row);
      ...
      #pragma region Procedure Tree node UI
         void _build_procedure_tree_branch_type_picker();
         void _build_procedure_type_picker();
