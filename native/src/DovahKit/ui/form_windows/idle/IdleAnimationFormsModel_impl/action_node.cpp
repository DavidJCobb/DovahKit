...
namespace IdleAnimationFormsModel_impl {
   /*virtual*/ size_t action_node::index_of_child(const node& child) const /*override*/ {
   ...
   /*virtual*/ const node* action_node::nth_child(size_t n) const /*override*/ {
      const auto& list = this->children.idles;
      if (n >= list.size())
         return nullptr;
      return list[n].get();
   }
   /*virtual*/ void action_node::update_cached_form_data() /*override*/ {
