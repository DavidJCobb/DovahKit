...
Options3DInputDialog::Options3DInputDialog(QWidget* parent) : QDialog(parent) {
   ...
   {  // tools
      ...
      widget->clear();
      widget->addItem(tr("None",        disamb), (int)DK3D::tools::id_of_none);
      widget->addItem(tr("Debug Log",   disamb), (int)DK3D::id_of_tool<DK3D::tools::debug_log>());
      widget->addItem(tr("Placeholder", disamb), (int)DK3D::id_of_tool<DK3D::tools::debug_placeholder>());
      widget->addItem(tr("Move Camera", disamb), (int)DK3D::id_of_tool<DK3D::tools::move_camera>());
      widget->addItem(tr("Turn Camera", disamb), (int)DK3D::id_of_tool<DK3D::tools::turn_camera>());
   }
   {  // input
      auto* widget = new DKBoundInputWidget(this);
...
/*static*/ Options3DInputDialog* Options3DInputDialog::open(QWidget* parent) {
   if (!current_instance)
      return new Options3DInputDialog(parent);
...
void Options3DInputDialog::_updateBindList() {
   ...
   for (int i = 0; i < size; ++i) {
      ...
      auto* tool = new QTableWidgetItem();
