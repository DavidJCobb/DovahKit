...
namespace dovah {
   ...
   void form_duplication_request::set_target(form_stub* original) {
      ...
      form_stub_helpers::for_each_child_form(original, [this, original](form_stub* child) {
         ...
         // fail due to the lack of a target parent form. (We run the check before cloning the 
         // parent, so we can't set the parent-clone as the parent of the child-clone to avoid 
