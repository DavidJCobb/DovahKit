...
#include <cassert>
#include <memory>
#include "./idles2/action_node.h"
#include "./idles2/action_root_candidacy.h"
#include "./idles2/graph_node.h"
#include "./idles2/idle_node.h"
#include "./idles2/loose_idle_list_node.h"
#include "./idles2/passkeys/initial_build.h"
#include "./idles2/passkeys/post_build_edit.h"
#include "./idles2/warning.h"
#include "./idles2/warnings/action_root_is_forced_loose.h"
#include "./idles2/warnings/child_idle_is_an_action_root_candidate.h"
#include "./idles2/warnings/child_of_idle_is_forced_loose.h"
#include "./idles2/warnings/cyclical_parent_relationships.h"
#include "./idles2/warnings/cyclical_sibling_relationships.h"
#include "./idles2/warnings/idle_has_candidacies_for_multiple_actions.h"
#include "./idles2/warnings/inconsistent_parentage_on_idle.h"
#include "./idles2/warnings/loose_idle_is_not_flagged.h"
#include "./idles2/warnings/no_loose_idle_list_for_idle.h"
#include "./idles2/warnings/orphaned_idle.h"
#include "./idles2/warnings/previous_sibling_is_not_as_expected.h"
#include "./idles2/warnings/sibling_is_an_ancestor.h"
#include "./idles2/warnings/siblings_have_mismatched_parents.h"
#include "../files/file_load_order.h"
#include "../forms/IdleAnimation.h"
#include "../form_stub.h"

namespace {
   using node                 = dovah::datastores::idles2::node;
   using action_node          = dovah::datastores::idles2::action_node;
   using graph_node           = dovah::datastores::idles2::graph_node;
   using idle_node            = dovah::datastores::idles2::idle_node;
   using loose_idle_list_node = dovah::datastores::idles2::loose_idle_list_node;

   using action_root_candidacy = dovah::datastores::impl::idles2::action_root_candidacy;

   namespace warnings {
      using namespace dovah::datastores::impl::idles2::warnings;
   }
...
namespace dovah::datastores {
   ...
   void idles2::build(file_load_order& lo) {
      this->reset();
      //
      ...
      std::sort(
         this->graphs.begin(),
         this->graphs.end(),
         [](const graph_node* a, const graph_node* b) {
      ...
      for (auto* graph : this->graphs) {
         std::sort(
            ...
            graph->actions.end(),
            [](auto* a, auto* b) {
               return cobb::stricmp_ascii(a->stub.get_editor_id(), b->stub.get_editor_id());
            }
         );
      }
      //
   ...
   graph_node* idles2::_get_or_create_graph(std::string_view path) {
   ...
   action_node* idles2::_get_or_create_action(std::string_view graph_path, form_stub* action) {
   ...
   void idles2::_place_action_root(idle_node& idle, loaded_idle_data& loaded) {
   ...
   void idles2::_place_forced_loose_idle(idle_node& idle, loaded_idle_data& loaded) {
   ...
   /*static*/ bool idles2::_idle_has_cyclical_parentage(std::set<form_stub*>& seen, loaded_idle_data& loaded) {
   ...
   /*static*/ idles2::sibling_problem idles2::_idle_has_bad_siblinghood(const std::set<form_stub*>& seen_ancestors, loaded_idle_data& loaded) {
   ...
   void idles2::_place_child_idle(idle_node& idle, loaded_idle_data& loaded) {
   ...
   void idles2::_post_placement_parentage_validation(idle_node& idle) {
   ...
   const graph_node* idles2::graph_by_path(std::string_view path) const noexcept {
      for (auto* graph : this->graphs)
         if (graph->path_equals(path))
            return graph;
      return nullptr;
   }
   graph_node* idles2::graph_by_path(std::string_view path) noexcept {
      return const_cast<graph_node*>(std::as_const(*this).graph_by_path(path));
   }
   const graph_node* idles2::graph_by_idle(idle_node& idle) const noexcept {
      return this->graph_by_path(idle.canonical_graph_path());
   }
   graph_node* idles2::graph_by_idle(idle_node& idle) noexcept {
      return const_cast<graph_node*>(std::as_const(*this).graph_by_idle(idle));
   }
   const graph_node* idles2::graph_by_idle(form_stub& stub) const noexcept {
      if (stub.form_type != form_type::idle)
         return nullptr;
      {
   ...
   graph_node* idles2::graph_by_idle(form_stub& stub) noexcept {
      return const_cast<graph_node*>(std::as_const(*this).graph_by_idle(stub));
   }

   const idle_node* idles2::idle_by_stub(form_stub& stub) {
      auto it = this->idles_by_stub.find(&stub);
      if (it == this->idles_by_stub.end())
         return nullptr;
      return it->second;
   ...
   bool idles2::is_idle_movement_legal(const idle_node& subject, const node& dst_parent, const idle_node* dst_previous) const {
   ...
   bool idles2::is_idle_movement_a_really_bad_idea(const idle_node& subject, const node& dst_parent, const idle_node* dst_previous) const {
   ...
   bool idles2::is_idle_deletion_legal(const idle_node&) const {
      return true;
   }
   bool idles2::is_idle_deletion_a_really_bad_idea(const idle_node& subject) const {
   ...
   #pragma region Helpers for editing operations
      void idles2::_on_runner_up_became_root(action_node& action) {
      ...
      void idles2::_destroy_non_canonical_active_root_candidacies(idle_node& subject) {
      ...
      idle_node* idles2::_take_idle_from_canonical_parent(node& take_from, idle_node& idle) {
      ...
      void idles2::_update_canonical_parent_action_after_root_taken(action_node& taken_from, idle_node& taken_idle) {
      ...
      void idles2::_update_form_data(idle_node& idle) {
   ...
   void idles2::_delete_single_idle(idle_node& subject) {
   ...
   void idles2::delete_idle(idle_node& subject) {
      assert(!!this->handlers.delete_idle);
   ...
   void idles2::move_idle(idle_node& subject, node& dst_parent, idle_node* dst_previous) {
      ...
      idle_node* former_next_sibling = this->_take_idle_from_canonical_parent(dst_parent, subject);
