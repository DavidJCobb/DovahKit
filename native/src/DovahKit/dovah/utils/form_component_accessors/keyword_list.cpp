...
namespace {
   using component_type = dovah::loaded_forms::components::keyword_list;
}

namespace {
   template<typename T>
   concept has_component = requires(T & x) {
      { x.keywords } -> std::same_as<component_type&>;
   };
...
namespace dovah::utils::form_component_accessors {
   extern const loaded_forms::components::keyword_list* keyword_list(const dovah::loaded_forms::Form& form) {
      return keyword_list(const_cast<dovah::loaded_forms::Form&>(form));
   }
   extern loaded_forms::components::keyword_list* keyword_list(dovah::loaded_forms::Form& form) {
   ...
   extern loaded_forms::components::keyword_list* keyword_list(dovah::form_stub& stub) {
      const size_t i = (size_t)stub.form_type;
      if (!presence.test(i))
         return nullptr;
      ...
      for (const auto& batch : mapping_list)
         for (size_t i = 0; i < form_types_per_batch; ++i)
            if (stub.form_type == batch.form_types[i])
               return batch.getters[i](*loaded);
      std::unreachable();
      return nullptr;
   }
}