...
namespace dovah {
   ...
   bsa_archived_file* bsa_archive::lookup_file(const std::string& path_and_name) const {
      if (path_and_name.empty())
         return nullptr;
      auto opt_hash_pair = bsa::utils::path_to_hashes(path_and_name);
      if (!opt_hash_pair.has_value())
         return nullptr;
      auto [folder_hash, file_hash] = opt_hash_pair.value();
      return this->lookup_file(folder_hash, file_hash);
   }
   ...
   bsa_archived_file* bsa_archive::read_contents_of(const bsa::packed_file_info& file_info) const {
      ...
      if (this->header.flags & bsa::archive_header::flag::embed_filenames) {
         ...
         } catch (bsa_unexpected_eof_exception&) {
            return nullptr;
         }
