...
namespace dovah::loaded_forms::structs::typed_package_info {
   class escort final : public base {
      public:
         static constexpr const legacy_type type = legacy_type::escort;
         static constexpr const uint32_t    header_subrecord = 'PKE2';

      public:
         std::optional<package_location> destination;
         std::optional<package_location> search_location;
         package_target target;
         uint32_t distance = 0; // PKE2+00

      public:
         virtual package_location* get_location_1() override { auto& o = this->destination; return o.has_value() ? &o.value() : nullptr; };
         ...
         //
         virtual void set_location_1(const package_location& v) override { this->destination = v; }
