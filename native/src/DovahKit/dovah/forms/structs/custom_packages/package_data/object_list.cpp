...
namespace dovah::loaded_forms::structs::custom_packages {
   ...
   /*static*/ void package_data_object_list::generate_use_info(tes_record_reader& record, form_stub_use_info_builder& uib) {
   };
   /*virtual*/ void package_data_object_list::save_value(tes_record_writer& record, load_order_interfaces::form_save& intfc) /*override*/ {
      auto& subrecord = record.open_next_subrecord('CNAM');
      subrecord.write(this->value);
      subrecord.close();
   }
   /*virtual*/ package_data* package_data_object_list::clone(loaded_forms::Form& owner_of_clone) const noexcept /*override*/ {
      auto  copy_ptr = std::make_unique<package_data_object_list>();
   ...
   /*virtual*/ void package_data_object_list::sever_outbound_references_to(form_stub& other, loaded_forms::Form& my_owner) noexcept /*override*/ {
   }
   /*virtual*/ void package_data_object_list::clear(loaded_forms::Form& my_owner) /*override*/ {
      this->value = false;
   }
}