...
computed_light calc_all_light_and_shadow(
   ...
   for(int i = 0; i < MAX_LIGHTS; ++i) {
      ...
      if (current.diffuse == vec3(0, 0, 0) && current.specular == vec3(0, 0, 0)) {
         continue;
      }
      //
      float shadow = 0.0;
      if (receive_shadows != 0 && rendered_light_can_cast_shadows(scene_lights[i])) { // if this light is allowed to cast shadows
