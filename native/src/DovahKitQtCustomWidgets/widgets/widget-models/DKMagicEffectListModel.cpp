...
#pragma region Item
void DKMagicEffectListModelItem::_update_cache() {
   ...
   {  // Magic School name
      ...
      if (i < 0) {
         this->cached.magic_school = DKMagicEffectListModel::tr("NONE", "actor vale name");
      } else if (i >= dovah::all_actor_value_info.size()) {
         this->cached.magic_school = DKMagicEffectListModel::tr("INVALID", "actor vale name");
      } else {
...
void DKMagicEffectListModel::importFrom(loaded_form& component_containing_form, const backend_type& component) {
   ...
   this->_max_effect_count = component.max_effect_count;
...
void DKMagicEffectListModel::_clear(bool signal) {
   if (signal)
      this->beginResetModel();
   for (auto* item : this->_items)
      delete item;
